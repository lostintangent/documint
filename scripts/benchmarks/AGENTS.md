...
- **Budget checks filter local-machine noise.** Benchmarks warm up first, run three times, and fail only after more than one over-budget run. The signal is repeated p99 pressure, not one slow sample.
...
- `index.ts` owns budgeted script orchestration, repeated-run pass/fail rules, and scenario-id/budget validation.
