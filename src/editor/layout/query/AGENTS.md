...
- **Line lookup is the geometry choke point.** `line-lookup.ts` owns the binary searches, region-line indices, boundary midpoint rules, and same-row disambiguation needed to map points and offsets back to prepared lines. Duplicating that logic risks table cells, wrapped lines, and end-offset cases drifting apart.
...
- `line-lookup.ts` owns line lookup, region-line disambiguation, and text-boundary offset/left projection.
