...
The navigation subsystem owns document-flow and layout-aware caret/selection movement, plus plain-text search — the set of editor capabilities whose job is "find positions in the document to navigate to." Keyboard motion (arrow keys, page movement, Home/End) and search differ only in how they pick the target: motion uses a direction, search uses a query string. Hit testing, click placement, and drag extension round out the point-of-action affordances.
