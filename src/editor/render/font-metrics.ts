...
export function resolveCanvasFontMetrics(font: string): CanvasFontMetrics {
  ...
  const fallbackEmHeight = Math.max(minimumFallbackEmHeight, Math.round(resolveCanvasFontSize(font)));

  if (!context) {
    const fallbackMetrics = {
      ascent: Math.round(fallbackEmHeight * 0.8),
      descent: Math.max(2, fallbackEmHeight - Math.round(fallbackEmHeight * 0.8)),
  ...
  const ascent =
    measuredHeight > 0
      ? measuredAscent
      : Math.round(emHeight * 0.8);
  const descent =
    measuredHeight > 0
      ? measuredDescent
      : Math.max(2, emHeight - ascent);
  const metrics = {
    ascent,
    descent,
    emHeight,
