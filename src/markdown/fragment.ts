/**
 * Markdown ↔ Fragment bridge. Mirrors `serializer.ts` / `parser/index.ts` at
 * the fragment altitude — no front matter, no comment threads, no trailing
 * newline.
 *
 * The `Fragment` type lives in the document subsystem alongside `Block` and
 * `Document`; this file owns only the markdown-text adaptation. The
 * classification is the same on both sides — `parseFragment` and
 * `extractFragment` both narrow to the lowest variant the payload's shape
 * allows so paste can take the most precise path on the way back.
 */
